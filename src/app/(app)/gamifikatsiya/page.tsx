...
export default function GamifikatsiyaPage() {
  ...
  return (
    <div className="space-y-6 pb-6">
      <div>
        <h1 className="text-2xl font-bold">{t("gam.sarlavha")}</h1>
        <p className="yumshoq">{t("gam.tavsif")}</p>
      </div>

      {/* Chellenj tugaganda avtomatik refleksiyaga o'tkazish */}
      {tugaganChellenj && (
        <div className="karta border-l-4 border-l-brand-500 p-5">
          <div className="font-semibold">{t("gam.tugadi", { nomi: tugaganChellenj.nomi })}</div>
          <p className="mt-1 text-sm yumshoq">{t("gam.tugadiMatn")}</p>
          <div className="mt-3 flex flex-wrap gap-2">
            <Link href="/maqsad" className="btn-asosiy">{t("gam.shaxsiyMaqsad")}</Link>
            <Link href="/profil" className="btn-ikkinchi">{t("gam.refleksiyaTugma")}</Link>
          </div>
        </div>
      )}
      ...
      {reyting.map((r) => (
        <div key={r.challengeId} className="karta p-5">
          <div className="flex items-center justify-between">
            <h2 className="font-bold">🏃 {r.nomi} {r.tugagan && <span className="text-sm yumshoq">{t("gam.yakunlandi")}</span>}</h2>
          </div>
          <p className="text-sm yumshoq">{t("gam.guruhTavsif")}</p>
          <div className="mt-4 space-y-3">
            {r.guruhlar.map((g, i) => (
              <div key={g.nomi}>
                <div className="flex items-center justify-between text-sm">
                  <span className={g.meniki ? "font-bold text-brand-700 dark:text-brand-300" : "font-medium"}>
                    {i === 0 ? "🥇 " : i === 1 ? "🥈 " : i === 2 ? "🥉 " : `${i + 1}. `}{g.nomi} {g.meniki && t("gam.meningGuruhim")}
                  </span>
      ...
      <div className="karta p-5">
        <h2 className="font-bold">{t("gam.faolHafta")}</h2>
        <p className="text-sm yumshoq">{t("gam.faolHaftaTavsif")}</p>
        <div className="mt-3 space-y-2">
          {FAOL_HAFTA_QOIDALARI.map((_, i) => (
            <label key={i} className={`flex cursor-pointer items-center gap-3 rounded-xl border p-3 transition ${faolHafta[i] ? "border-ok bg-ok/5" : ""}`} style={!faolHafta[i] ? { borderColor: "var(--chegara)" } : {}}>
              ...
              <span className={faolHafta[i] ? "font-medium" : ""}>{t(`faolHafta.${i + 1}`)}</span>
              {faolHafta[i] && <span className="ml-auto text-ok">✓</span>}
            </label>
          ))}
        </div>
        <div className="mt-3 text-sm yumshoq">{t("gam.bugun", { n: faolHafta.filter(Boolean).length })}</div>
      </div>

      {/* Nishonlar */}
      <div className="karta p-5">
        <h2 className="font-bold">{t("gam.nishonlar")}</h2>
      ...
      <div className="karta border-l-4 border-l-warn p-4 text-sm yumshoq">{t("gam.qoidalar")}</div>
    </div>
  );
}
